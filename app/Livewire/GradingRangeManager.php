...
class GradingRangeManager extends Component
{
    ...
    public function saveAllRanges()
    {
        $this->isLoading = true; // Show loading state
        $this->validateRanges();

        foreach ($this->grades as $subjectId => $gradeRange) {
            GradingRange::create([
                'range_from' => $gradeRange['range_from'],
                'range_to' => $gradeRange['range_to'],
                'grade' => '', // Initialize grade field
                'remark' => '',
                'gpa' => '',
                'grading_system_id' => $this->selectedGradingSystem,
                'subject_id' => $subjectId, // Include subject ID in the create method
            ]);
        }
        ...
        $this->alert('success', 'Grading ranges saved successfully for all subjects!');
    }
