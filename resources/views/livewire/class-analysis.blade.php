<div class="card  p-3 shadow-lg border rounded" style="background: linear-gradient(135deg, #f8f9fa, #e9ecef);">
    ...
    {{-- <div wire:loading wire:target="examId" class="text-center">
        <div class="spinner-border text-primary" role="status">
            ...
            {{--<span wire:loading wire:target="examId"><i class="fas fa-spinner fa-spin"></i>
                Loading grades...</< /span>
                {{-- <span wire:loading.remove wire:target="examId"><i class="fas fa-plus-circle"></i></span> --}}
        {{-- </div> --}}
    {{-- </div>  --}}

    <div wire:click="examId"  wire:loading.attr="disabled">
        {{-- <span wire:loading.remove wire:target="examId">Load Exams</span> --}}
    ...
    <h3 class="mt-4 text-primary font-weight-bold">Analysis for Exam: <span class="text-success">{{ $examName }}</span>
        - Class: <span class="text-success">{{ $className }}</span></h3>
    @endif
    ...
    <table class="table table-bordered table-hover mt-4" style="border-radius: 0.5rem; overflow: hidden;">
        ...
        <tbody>
            ...
            <tr>
                @if ($loop->first)
                <td rowspan="{{ $sectionCount }}">{{ $classData['class_name'] }}</td>
    ...
    <div class="alert alert-info mt-4" style="border-radius: 0.5rem; font-weight: bold;">
        <strong>Note:</strong> No grades data found for the selected exam: <span class="text-success">{{ $examName
            }}</span>.
    </div>
    @endif
