<div class="container mt-4" x-data="{ showInstruction: true }">
    <h2 class="mb-4 text-center">Assign Exam Marks</h2>
    ...
    <template x-if="showInstruction && '{{ $selectedClass }}'">
        <div class="alert alert-info mt-3" 
             x-init="$el.style.transition = 'opacity 0.5s';" 
             x-show="showInstruction" 
    ...
    @if ($selectedExam)
        @if ($students->isNotEmpty())
            <div class="alert alert-info mt-3" x-show.transition.opacity="!showInstruction">
                You are now assigning marks for <strong>{{ $selectedExamName }}</strong> in <strong>{{ $selectedClassName }}</strong>.
            </div>
