<div class="container mt-2" wire:poll.10s>
    <div>
                ...
                @forelse ($suspendedStudents as $student)
                    <div class="col-lg-6 col-md-6 col-sm-12 mb-4">
                        ...
                        <div class="card p-3 shadow-lg border rounded"
                            ...
                            <div class="card-body">
                                <h3 class="text-center font-weight-bold mb-2">{{ $student->first_name }}
                                    {{ $student->last_name }}</h3>

                                <p class="mb-2"><strong>Admission Number:</strong> <span
                                        class="badge bg-primary">{{ $student->adm_no }}</span></p>
                                <p class="mb-2"><strong>Suspension Reason:</strong> <span
                                        class="badge bg-danger">{{ $student->suspension_reason }}</span></p>
                                <p class="mb-2"><strong>Suspension Type:</strong> <span
                                        class="text-muted">{{ ucfirst($student->suspension_type) }}</span></p>
                                <p class="mb-2"><strong>Suspension Date:</strong> <span
                                        class="text-muted">{{ $student->suspension_date ? $student->suspension_date->format('Y-m-d') : 'N/A' }}</span>
                                </p>
                                <p class="mb-2"><strong>Suspension End Date:</strong> <span
                                        class="text-muted">{{ $student->suspension_end_date ? $student->suspension_end_date->format('Y-m-d') : 'N/A' }}</span>
                                </p>
                                ...
                                <div class="alert alert-info mt-2">
                                    <p class="mb-1"><strong>Remaining Days:</strong>
                                        @if ($student->suspension_end_date)
                                            <span
                                                class="text-success">{{ $this->humanReadableCountdown($student->suspension_end_date) }}</span>
                                        @else
                                    ...
                                    <p class="mb-0"><strong>Time Since Suspension:</strong>
                                        @if ($student->suspension_date)
                                            <span
                                                class="text-muted">{{ $this->humanReadableElapsedTime($student->suspension_date) }}</span>
                                        @else
                                            <span class="text-muted">N/A</span>
                                        @endif
                                    </p>
            ...
            <div class="mt-4">
                ...
                <div class="d-flex gap-3">
                    <div class="d-flex align-items-center">
                        ...
                        <div wire:loading wire:target="confirmExtension" class="spinner-border spinner-border-sm ms-2"
                            role="status"></div>
                    </div>
