<div x-data="{ showSample: false, progress: 0, isUploading: false, showErrorTable: @entangle('showEditTable') }" class="card mt-3 p-3 shadow-lg border rounded"
    ...
    <div class="card-body">
        ...
        <!-- File Upload Form -->
        <form wire:submit.prevent="importStudents" x-ref="form" @submit="isUploading = true">
            <div class="mb-4">
        ...
        @if (count($errorDetails) > 0)
            <div class="alert alert-warning mt-4">
                ...
                <button @click="showErrorTable = !showErrorTable" class="btn btn-warning">Correct Errors</button>
            </div>
        @endif
                            ...
                            @foreach ($editableRows as $index => $row)
                                <tr>
                                    ...
                                    <td><input type="text"
                                            ...
                                            class="form-control" @if (!in_array('adm_no', $row['errors'])) disabled @endif></td>
                                    <td><input type="text"
                                            wire:model="editableRows.{{ $index }}.values.first_name"
                                            class="form-control" @if (!in_array('first_name', $row['errors'])) disabled @endif></td>
                                    <td><input type="text"
                                            wire:model="editableRows.{{ $index }}.values.middle_name"
                                            class="form-control" @if (!in_array('middle_name', $row['errors'])) disabled @endif></td>
                                    <td><input type="text"
                                            wire:model="editableRows.{{ $index }}.values.last_name"
                                            class="form-control" @if (!in_array('last_name', $row['errors'])) disabled @endif></td>
                                    <td><input type="text"
                                            wire:model="editableRows.{{ $index }}.values.gender"
                                            class="form-control" @if (!in_array('gender', $row['errors'])) disabled @endif></td>
                                    <td><input type="date" wire:model="editableRows.{{ $index }}.values.dob"
                                            class="form-control" @if (!in_array('dob', $row['errors'])) disabled @endif></td>
                                    <td><input type="text"
                                            wire:model="editableRows.{{ $index }}.values.class_name"
                                            class="form-control" @if (!in_array('class_name', $row['errors'])) disabled @endif></td>
                                    <td><input type="text"
