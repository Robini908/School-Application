    ...
    @if ($subjectId && $selectedGradingSystem)
        <div x-data="{ moreDetails: false }" class="mb-4">
            <div class="alert alert-info shadow p-4 rounded">
                ...
                <p class="mb-1">Grading System:
                    <strong>{{ $gradingSystems->find($selectedGradingSystem)->name }}</strong>
                </p>
        ...
        <div>
            ...
            @if ($activeAction === 'reuseSame')
                <div class="alert alert-info">
                    ...
                    <p>Select a subject from the dropdown to reuse grading ranges that are already established in the
                        system.
                    </p>
        ...
        <div class="my-4 inline-block justify-between">
            @if (!$isEditing)
                @if (count($ranges) < 12)
        ...
        @if ($submittedRanges->isEmpty())
            <div class="alert alert-warning">No grading ranges submitted for this subject and grading
                system.<br><br>Consider
        ...
        @else
            <div class="card my-4 p-4 shadow-lg">
                ...
                <p class="card-text text-muted">
                    <strong>Grading System:</strong> <span
                        class="font-weight-normal">{{ $submittedRanges->first()->gradingSystem->name }}</span>
                </p>
                <p class="card-text text-muted">
                    <strong>Subject:</strong> <span
                        class="font-weight-normal">{{ $submittedRanges->first()->subject->subject_name }}</span>
                </p>
                                    ...
                                    <td class="text-center">
                                        <div class="d-flex justify-content-center">
                                            ...
                                            <button wire:click="editSubmittedRange({{ $submittedRange->id }})"
                                                ...
                                                <span wire:loading
                                                    wire:target="editSubmittedRange({{ $submittedRange->id }})">
                                            ...
                                            <button wire:click="deleteRange({{ $submittedRange->id }})"
                                                ...
                                                <span wire:loading
                                                    wire:target="deleteRange({{ $submittedRange->id }})">
                                                    <i class="spinner-border spinner-border-sm" role="status"
                                                        aria-hidden="true"></i>
                                                </span>
                                            </button>
